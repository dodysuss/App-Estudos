...
import { Archive, Boxes, Plus, Search, Sparkles } from "lucide-react";
...
type SearchParams = Promise<{ search?: string; semantic?: string; category?: string; tag?: string | string[]; archived?: string; folder?: string }>;
...
export default async function AssetsPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  const categories = uniqueSorted(assets.map((asset) => asset.category));
  const tags = uniqueSorted(assets.flatMap((asset) => asset.tags));
  ...
  return (
    <div className="page-shell">
      ...
      <form className="surface-card p-4 md:p-5" method="get">
        ...
        <div className="mb-4">
          <h2 className="text-sm font-semibold">Refinar ativos</h2>
          <p className="text-xs text-muted-foreground">Busca textual, busca semântica, categoria e tags.</p>
        </div>
        <div className="grid gap-3 lg:grid-cols-2">
          <label className="relative">
            <Search className="absolute left-3.5 top-3 h-4 w-4 text-muted-foreground" />
            <Input name="search" defaultValue={params.search ?? ""} placeholder="Busca textual por nome, tag, categoria ou conteúdo..." className="pl-10" />
          </label>

          <label className="relative">
            <Sparkles className="absolute left-3.5 top-3 h-4 w-4 text-muted-foreground" />
            <Input name="semantic" defaultValue={params.semantic ?? ""} placeholder="Busca semântica: objetivo, ideia, problema ou tema relacionado..." className="pl-10" />
          </label>
